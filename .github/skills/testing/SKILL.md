---
name: Testing
description: テスト戦略（AAAパターン、Builderパターン、カバレッジ基準）、Vitestを用いた単体テスト・統合テストの実装ガイドライン。
---
...
## Quick Start
1. 実装対象のファイルに対応するテストファイル（`*.test.ts` / `*.spec.ts`）を作成する。
2. Guidelinesの「1. テストコードの構造（AAAパターン）」に従ってテストケースを記述する。
3. `vitest` コマンドでテストを実行し、パスすることを確認する。
...
## Templates
- Path: `.github/skills/testing/templates/`
