---
name: Testing
description: 信頼性の高いテストコードを書くための指針。AAAパターン、Builderパターンによるデータ生成、正常系・異常系の網羅、そしてビジネスロジックを重視したカバレッジ戦略について解説します。
---
...
*   新しい機能を実装した際
*   バグ修正を行い、再発防止を確実にする際
*   リファクタリングを行い、既存の挙動を壊していないか確認する際
*   仕様変更による影響範囲を特定する際

## Quick Start

1.  **ファイルの作成**: テスト対象のファイル（例: `user.ts`）に対応するテストファイル（例: `user.test.ts`）を作成します。
2.  **テストケースの定義**: `describe` ブロックでグループ化し、`it` ブロックで具体的なテストケースを記述します。
3.  **AAAパターンの適用**: テスト内を `Arrange`（準備）、`Act`（実行）、`Assert`（検証）の3ステップに分けて記述します。
4.  **実行と確認**: テストランナー（Vitestなど）を実行し、テストがパスすることを確認します。
...
*   `templates/`: テストコードやBuilderパターンのテンプレートが含まれます。
