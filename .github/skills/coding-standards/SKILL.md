...
description: 高品質なコードを維持するための原則と規約。DRY, KISS, YAGNI, SOLID原則の詳細な解説と、TypeScriptにおける命名規則や実践的なベストプラクティスを提供します。
---
...
*   新しいコードを実装する際
*   Pull Requestのコードレビューを行う際
*   既存コードのリファクタリングや改善を行う際
*   変数名や関数名、クラス設計に迷った際

## Quick Start

1.  **原則の確認**: コーディングを開始する前に、SOLIDやDRYなどの基本原則を意識します。
2.  **自動化ツールの活用**: LinterやFormatterを有効にし、スタイルガイドに準拠しているか自動チェックします。
3.  **命名の徹底**: 変数や関数には、その役割と意図が明確に伝わる名前を付けます。
4.  **ボーイスカウトルール**: 既存コードを修正する際は、発見した些細な問題も修正してコミットします。
...
*   `templates/`: コーディング規約に関連するテンプレートファイルが含まれます。
